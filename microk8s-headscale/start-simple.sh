...
: "${HEADPLANE_COOKIE_SECRET:?Set HEADPLANE_COOKIE_SECRET in .env}"
: "${HEADSCALE_API_KEY:?Set HEADSCALE_API_KEY in .env}"
export HEADSCALE_NAMESPACE INGRESS_CLASS_NAME TLS_SECRET_NAME HEADSCALE_IMAGE HEADPLANE_IMAGE HEADSCALE_HOST HEADPLANE_HOST HEADSCALE_DB_HOST HEADSCALE_DB_PORT HEADSCALE_DB_NAME HEADSCALE_DB_USER HEADSCALE_DB_PASSWORD HEADPLANE_COOKIE_SECRET HEADSCALE_API_KEY
...
for f in headscale/deployment.yaml headscale/service.yaml headscale/pvc.yaml headplane/deployment.yaml headplane/service.yaml headplane/pvc.yaml; do
